...
def solve(&block)
  open("01.in").read.scan(/\d+/).map {|m| yield(m.to_i) }.reduce(:+)
end
...
# requires fuel, and that fuel requires fuel, and so on.  Any mass
# that would require negative fuel should instead be treated as if it
