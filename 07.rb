...
# the thrusters by trying every possible combination of phase settings
# on the amplifiers.  Make sure that memory is not shared or reused
# between copies of the program.
#
...
Program = open("07.in").read.scan(/-?\d+/).map(&:to_i)
...
class Amplifier
  ...
  def run(input)
    # Runs the program; reads a single input signal and stops after
    # returning a single output signal.  For the benefit of part 2,
    # may be called again to continue execution of the program.
    if !@phase.nil?
      input = [input, @phase]
      @phase = nil
    else
    ...
    param = lambda {|i|
      if (@program[@ip]/10**(i+1))%10 == 0
        @program[@program[@ip+i]]
      else
        @program[@ip+i]
      end
    }
    while true
      ...
      when 2
        ...
        @ip += 4
      when 3
        @program[@program[@ip+1]] = input.pop
        @ip += 2
      when 4
        output = param[1]
        @ip += 2
      ...
      when 6
        @ip = (param[1] == 0 ? param[2] : @ip+3)
      when 7
        @program[@program[@ip+3]] = (param[1] < param[2] ? 1 : 0)
        @ip += 4
      when 8
        @program[@program[@ip+3]] = (param[1] == param[2] ? 1 : 0)
        @ip += 4
...
puts (0..4).to_a.permutation.map {|p|
  s = 0
  p.each do |i|
    s = Amplifier.new(i).run(s)
  end
  s
}.max
...
# each amplifier its phase setting at its first input instruction; all
...
# To start the process, a 0 signal is sent to amplifier A's input
# exactly once.
#
...
# --------------------
#
# We assume that all amplifiers halt at the same time (the puzzle
# wouldn't really work if they didn't).

puts (5..9).to_a.permutation.map {|p|
  amps = p.map {|i| Amplifier.new(i) }
  i = 0
  s = 0
  final_s = nil
  while true
