...
# top-left pixel of the first layer, the second digit corresponds to
# the pixel to the right of that on the same layer, and so on until
# the last digit, which corresponds to the bottom-right pixel of the
# last layer.
#
...
image = open("08.in").read.chomp.chars.map(&:to_i)

puts image.each_slice(LS).map {|l|
  counts = [0, 0, 0]
  l.each do |p|
    raise if ![0, 1, 2].member?(p)
    counts[p] += 1
  end
  counts
}.min[1, 2].reduce(:*)
...
# the first and second layers, a black pixel in the third layer, and a
# white pixel in the fourth layer, the final image would have a black
...
composite = [nil]*LS
image.each_slice(LS).to_a.reverse.each do |l|
