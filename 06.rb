...
# it wasn't corrupted during the download.  To verify maps, the
...
class Node
  ...
  def Node.fetch(name)
    @@all.member?(name) ? @@all[name] : Node.new(name)
  end
...
open("06.in").each do |l|
  p, c = l.chomp.split(")")
  Node.fetch(p).has_child(Node.fetch(c))
end

root = Node.all.values.find {|n| n.parent.nil? } # i.e., COM

def walk(node, depth)
  depth + node.children.map {|c| walk(c, depth+1) }.reduce(0, :+)
end
...
class Node
  def path
    (@parent.nil? ? [] : @parent.path) + [self]
  end
end

you = Node.all["YOU"].path
san = Node.all["SAN"].path
i = you.zip(san).index {|y,s| y != s } - 1 # index of last common ancestor
puts (you.length-i-2) + (san.length-i-2)
