...
# top edge (so the top-left corner is 0,0 and the position immediately
# to its right is 1,0).
#
...
# --------------------
#
# For each asteroid A we build a map of all other asteroids expressed
# in polar coordinates relative to A, grouped by angle.  The map takes
# the form of a hash
# { angle => [[distance squared, [x,y]], ...], ... }.

require "set"

asteroids = Set.new
open("10.in").each_with_index do |r,y|
  r.chomp.chars.each_with_index do |c,x|
    asteroids.add([x,y]) if c == "#"
  end
end

best_map = asteroids.map {|a|
  map = Hash.new {|h,k| h[k] = [] }
  asteroids.reject {|b| b == a }.each do |b|
    dx, dy = b[0]-a[0], b[1]-a[1]
    # The special case below is needed because otherwise atan2 returns
    # positive pi, which will cause problems in part 2.  For our
    # puzzle data, dividing by the gcd is not necessary, but in
    # general it would seem a good prophylactic against roundoff error
    # since we are using computed floating point numbers as hash keys.
    if dx == 0 && dy < 0
      angle = -Math::PI
    else
      gcd = dx.gcd(dy)
      angle = Math.atan2(-dx/gcd, dy/gcd)
    end
    map[angle] << [dx*dx+dy*dy, b]
  end
  map
}.max_by {|m| m.length }

puts best_map.length
...
best_map.each_value do |l|
  l.sort!
end

n = 0
while best_map.length > 0
  best_map.keys.sort.each do |angle|
    l = best_map[angle]
    n += 1
    if n == 200
      a = l[0][1]
    ...
    l.delete_at(0)
    best_map.delete(angle) if l.length == 0
  end
end
