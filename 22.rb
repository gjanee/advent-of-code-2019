...
# ship.  At least you're drifting in the right direction.  You decide
...
# top card of the deck onto the top of the new stack repeatedly until
...
# table to lay out all of the cards individually in a long line.  Deal
# the top card into the leftmost position.  Then, move N positions to
# the right and deal the next card there.  If you would move into a
# position past the end of the space on your table, wrap around and
# keep counting from the leftmost card again.  Continue this process
...
# below the top card, and so on, until the rightmost card ends up at
...
# combine techniques; they all start with a factory order deck of 10
# cards:
#
...
# card immediately below the top card, and so on to the bottom.  (That
# is, cards start in the position matching their number.)
#
...
eval(
  "def shuffle(deck)\n" +
  open("22.in").read
    .gsub(/deal into new stack/, "deck = deal_into_new_stack(deck)")
    .gsub(/deal with increment (\d+)/, "deck = deal_with_increment(deck, \\1)")
    .gsub(/cut (-?\d+)/, "deck = cut(deck, \\1)") +
  "end")

puts shuffle((0...10007).to_a).index(2019)
...
# you think you see Halley's Comet fly past!
#
...
# --------------------
#
# This part needs to be solved largely mathematically.  If we think of
# the repeated application of the composition of the operations as a
# permutation function f, we seek to compute the inverse function
# finv(2020).  For a deck of n cards, the inverse function finv(i) of
# each individual operation is:
#
# reverse: n-1-i
# cut k cards: i+k
# increment by k: (1/k)*i
#
# These functions are all modulo n.  In the last case, 1/k denotes the
# modular inverse of k.  Notice that these functions can always be
# expressed, including when combined, in the simple linear form a*i+b
# for constants a and b.  For input a*i+b, finv(a*i+b) can be computed
# as:
#
# reverse: -a*i + n-1-b
# cut k cards: a*i + b+k
# increment by k: (a*(1/k))*i + b*(1/k)
#
# Applying a permutation function of the form a*i+b multiple times
# yields
#
# a*i + b
# a^2*i + a*b + b
...
# ...
#
# and generally after m iterations,
#
# a^m*i + b*(a^m-1)/(a-1)
#
# We thus walk through the operations in reverse order to compute
# constants a and b, then apply the last formula for i = 2020.
...
def power(a, e, n)
  # (a**e)%n where e may be very large
  r = 1
  p = a
  while e > 0
    r = (r*p)%n if e%2 == 1
...
a, b = 1, 0
open("22.in").readlines.reverse.each do |l|
  if /deal into new stack/.match(l)
    a, b = -a, N-1-b
  elsif /deal with increment (\d+)/.match(l)
    i = inverse(Regexp.last_match[1].to_i, N)
    a, b = a*i, b*i
  elsif /cut (-?\d+)/.match(l)
...
puts (power(a,E,N)*2020 + b*(power(a,E,N)-1)*inverse(a-1,N))%N
