...
Wires = open("03.in").readlines

def walk(wire, &block)
  # Yields wire grid positions to an optional block; returns an array
  # of all non-nil values returned by the block.
...
wire0 = Set.new(walk(Wires[0]))

puts walk(Wires[1]) {|p| wire0.member?(p) ? p : nil }
...
# is reached after 8+5+5+2 = 20 steps by the first wire and
