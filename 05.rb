...
# The Elves suggest that you get the air conditioner working by
# upgrading your ship computer to support the Thermal Environment
...
#   instruction 3,50 would take an input value and store it at address
#   50.
...
# interpreted as a value - if the parameter is 50, its value is simply
# 50.
#
...
# 0 (hundreds digit), 1 (thousands digit), and 0 (ten-thousands digit,
...
Program = open("05.in").read.scan(/-?\d+/).map(&:to_i)

def run(input)
  input = input.reverse
  p = Program.clone
  ip = 0
  param = lambda {|i| (p[ip]/10**(i+1))%10 == 0 ? p[p[ip+i]] : p[ip+i] }
  output = []
  while true
    case p[ip]%100
    when 1
      p[p[ip+3]] = param[1] + param[2]
      ip += 4
    when 2
      p[p[ip+3]] = param[1] * param[2]
      ip += 4
    ...
    when 4
      output << param[1]
      ip += 2
    when 5 # additional opcodes defined in part 2...
      ip = (param[1] != 0 ? param[2] : ip+3)
    when 6
      ip = (param[1] == 0 ? param[2] : ip+3)
    when 7
      p[p[ip+3]] = (param[1] < param[2] ? 1 : 0)
      ip += 4
    when 8
      p[p[ip+3]] = (param[1] == param[2] ? 1 : 0)
      ip += 4
