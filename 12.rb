...
# of 3, and Callisto has an x position of 5, then Ganymede's x
...
# sum of the absolute values of its x, y, and z position coordinates.
...
class Triple < Array
  # a triple of numbers representing position or velocity
  ...
  def +(other)
    Triple.new((0..2).map {|i| self[i]+other[i] })
  end
...
def load
  open("12.in").map {|l|
    Moon.new(Triple.new(l.scan(/-?\d+/).map(&:to_i)))
  }
end
...
def step
  (0..2).each do |i|
    $moons.each do |m|
      others = $moons - [m]
      m.velocity[i] +=
        others.count {|o| o.position[i] > m.position[i] } -
        others.count {|o| o.position[i] < m.position[i] }
    end
  end
  $moons.each do |m|
...
# --------------------
#
# The motion of the moons is reversible.  Given a moon's current
# position and velocity, its previous position can be computed since
# the current velocity was applied to the previous position.  And
# given previous positions, previous gravity can be computed and, from
# that, previous velocities.  Therefore the states of the system must
# form a simple cycle (*), and therefore we can look for the first
# repeat of the initial state.
#
# (*) Assuming that moon positions are bounded.  Although we have no
# proof of that, it does seem that gravity will prevent moons from
# flying arbitrarily far apart.
#
# The dimensions are independent, hence we calculate the cycle length
# for each dimension separately (in parallel) and then compute the
# least common multiple of the lengths.

class Moon
  def state(i)
    # the state of the moon along axis i
    [@position[i], @velocity[i]]
  end
end

def state(i) # global state along axis i
  $moons.map {|m| m.state(i) }
end

$moons = load

seen_states = (0..2).map {|i| Set.new([state(i)]) }
n = 0
cycles = [nil]*3 # cycle lengths
while !cycles.all?
  n += 1
  step
  (0..2).each do |i|
    if cycles[i].nil?
      s = state(i)
      cycles[i] = n if seen_states[i].member?(s)
      seen_states[i].add(s)
    end
  end
end
