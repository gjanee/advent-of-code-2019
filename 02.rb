...
# should read the values at positions 10 and 20, add those values, and
# then overwrite the value at position 30 with their sum.
#
...
# Together, they represent the first opcode (1, addition), the
...
# at position 3, so it overwrites itself.  Afterward, the program
# looks like this:
#
# 1,9,10,70,
# 2,3,11,0,
...
Program = open("02.in").read.scan(/\d+/).map(&:to_i)

def run(noun, verb) # arguments will become apparent in part 2
  p = Program.clone
  p[1] = noun
  p[2] = verb
  ip = 0
...
# an Intcode program, make sure to start by initializing memory to the
...
# 1 is the opcode; 2, 3, and 4 are the parameters.  The instruction 99
...
# --------------------
#
# Fun Easter egg: 19690720 is the date of the first moon landing, and
# the Apollo 11 astronauts inputted two numbers, called "verb" and
# "noun," to the guidance computer to direct which program to run.

(0..99).each do |noun|
  (0..99).each do |verb|
    puts 100*noun+verb if run(noun, verb) == 19690720
  end
end
